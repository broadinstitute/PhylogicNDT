...
class DpEngine:
    ...
    def __init__(self,data, N_iter, Pi_k, use_fixed=False,co_assign_flag=False,ignore_nan=False,tsne=True,mode=None):
        ...
        if ignore_nan:
            logging.error("Ignore nan specified, but the input should be a CCF histogram class which controls for NaNs internally. This functionailty is redundant, and removed.")
        if co_assign_flag:
        ...
        s_cluster=DP_cluster(self,DP_item(mut_iter.next()[0],self))
        ...
        cluster_out=self.tree_cluster_DP(co_assign_flag,K,var_loc=var_loc,tsne=tsne,grid_size=self.num_bins) #co-assignment matrix is global member of Mut class.
        ...
        for key,value in cluster_out.iteritems():
    ...
    def one_iteration(self,resample=True):
        for mut in self.mutations:
            ...
            for cluster_idx,cluster in enumerate(self.clusterlist):
                ...
                if mut not in cluster: #TODO: redefine in 
                    ...
                    loglik[cluster_idx] = self.logsum_of_marginals_per_sample(self.normalize_loghist_with_prior(cluster-mut) + mut.loghist)

            open_new_clust_const= self.alpha/float(self.n_muts - 1 + self.alpha)
            
            prior=np.clip(np.exp(self.logprior)-np.exp(reduce(lambda x,y:np.maximum(x,y),[z.normed_hist for z in self.clusterlist])),a_min=1e-40,a_max=1.)


            loglik[-1]=self.logsum_of_marginals_per_sample(mut.loghist+self.normalize_loghist_with_prior(np.log(prior)))
        ...
        self.results.cluster_positions.append([[np.argmax(x) for x in cluster.normed_hist] for cluster in self.clusterlist])
        ...
        if resample:
            ...
            self.alpha=sample_gamma_cond_N_k(self.n_muts, self.n_clusters, self.eta, self.gamma_prior) ## Escobar and West 1995
    ...
    def one_iteration_fix_k(self):
        ...
        return [real_index[x.assigned_to.id] for x in self.mutations],[cluster.normed_hist for cluster in self.clusterlist]
    


    def tree_cluster_DP (self, co_assign,K,clonal_threshold=0.95,var_loc=None,DP_res=None,N_burn=0,tsne=True,grid_size=101):
        ...
        assign = scipy_cluster.hierarchy.fclusterdata(var_loc,K,criterion='maxclust',metric="cityblock",method="average")
        #if tsne:
        ...
        for i,new_cluster in enumerate(CL):
            log_f_post=get_norm_marg_hist(np.reshape(np.sum(log_data_flattened[assign==new_cluster,:],0,dtype=np.float32),(self.n_samples,self.num_bins)), self.logprior)
            cluster_dens.append(np.exp(log_f_post))

            mcmc_clust_dens.append(np.reshape(np.sum(var_loc[assign==new_cluster,:],axis=0),(self.n_samples,self.num_bins)))
        ...
        _,mcmc_clust_dens_sorted=zip(*sorted(enumerate(mcmc_clust_dens),key=lambda x:cluster_max[x[0]],reverse=True))
    ...
    def logsum_of_marginals(self,loghist):
        return np.sum(np.apply_along_axis(lambda x: logsumexp(x), 1, loghist))
    def logsum_of_marginals_per_sample(self,loghist):
...
class DP_item:
    def __init__(self,label,engine,assignment=None,concordant_with=set(),discordant_with=set()):
        ...
        self.item_prior=self._DP_engine_parent.logsum_of_marginals_per_sample(self.loghist+self._DP_engine_parent.logprior)[0]
...
class DP_cluster:
    ...
    def __hash__(self):
        return self.id
    def __eq__(self,other):
        return self._muts == other._muts ## A mutation should never be in more than one cluster. 
...
class ClusteringResults:
    ...
    def _create_from_dict(cls,legacy_dict):
        #logging.warning("!!! Creating results object from dictionary, this functionailty will be removed. !!!")
        res_obj=cls()
    ...
    def __init__(self,assign=None,clust_CCF_dens=None,tSNE=None,mcmc=None,var_loc=None):
        ...
        self.eta=[]
        self.alpha=[]
        self.assign = []  ### this is a dupilciate parameter, the overwrite is intentional. Save the assign. across samples.
...
def summarize_mut_locations( DP_res, N_burn):
    ...
    sorted_counts=np.array(cluster_counts.items())
...
#   gamma_GRID = seq(0.01,10, length.out = 1000)
   ...
   for i in range(N_gamma):
      ...
      DP_prob[i,:] = DP_prob_k_cond_alpha_N(N, gamma_GRID[i], log_stirling_coef)   ## Escobar and West 1995 for DP loglik eqn 10.
...
def init_dp_prior(N,Pi_k):
   k_prior = stats.nbinom.pmf(range(1,N+1),Pi_k["r"],Pi_k["r"]/float(Pi_k["r"]+Pi_k["mu"])) #stats.nbinom.pmf(range(1,N+1),Pi_k["mu"]+ Pi_k["mu"]**2/float(Pi_k["r"]), Pi_k["r"]/float(Pi_k["r"]+Pi_k["mu"]) )
