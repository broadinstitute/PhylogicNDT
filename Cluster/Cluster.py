...
def run_tool(args):
        ...
        if args.sif:  # if sif file is specified
            ...
            for file_idx,file_line in enumerate(sif_file):
                ...
                if not file_idx: continue
                if file_line.strip('\n').strip == "": continue #empty rows
            ...
            for idx, sample_entry in enumerate(args.sample_data):
                ...
                patient_data.addSample(maf_fn, sample_id,timepoint_value=timepoint, grid_size=args.grid_size, _additional_muts=None,
                                        seg_file=seg_fn,
                                        purity=purity)
                if len(patient_data.sample_list) == args.n_samples: #use only first N samples
                    break
