import os
import sys
from .Enums import Enum, MutStatus, MutType
import logging
...
class SomMutation:
    ...
    def __init__(self, chrN, pos, ref, alt, ccf_1d, ref_cnt=None, alt_cnt=None, gene=None, prot_change=None,
        ...
        self.status = MutStatus.OK  # later if blacklisted or graylisted
        ...
        if type_ is None:
            ...
            if "-" in self.ref:
                self.type = MutType.INS
            elif "-" in self.alt:
                self.type = MutType.DEL
            else:
                self.type = MutType.SNV
        else:
            self.type = type_
