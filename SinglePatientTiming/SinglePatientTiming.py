    ...
    if args.sif:  # if sif file is specified
        with open(args.sif, 'r') as sif_file:
            ...
            for line in sif_file:
                ...
                patient_data.addSample(maf_fn, sample_id, seg_input_type='timing_format', timepoint_value=timepoint,
                                       grid_size=101, _additional_muts=None, seg_file=seg_fn, purity=purity)
...
def compare_events(timing_engine, drivers=()):
    ...
    all_events.extend(itertools.chain(*timing_engine.truncal_cn_events.values()))
    all_events.extend(mut for mut in timing_engine.mutations.values() if mut.prot_change and mut.gene in drivers)
    comps = {}
    for eve1, eve2 in itertools.combinations(all_events, 2):
        ...
        comps[(eve1.event_name, eve2.event_name)] = (prob_event1_before_event2, prob_event2_before_event1, prob_unknown)
    return comps
